...
;(async () => {
  ...
  /** @type {{ fabric: string[], forge: string[], neoforge: string[], all: string[] }} */
  ...
  /** @type {Promise<{ mcVersion: string, fabricVersion: string }[]>} */
  const fabricVersions = doFabric
  ...
  /** @type {Promise<{ mcVersion: string, forgeVersion: string }[]>} */
  const forgeVersions = doForge
    ? (async () => {
  ...
  /** @type {Promise<{ mcVersion: string, neoforgeVersion: string }[]>} */
  const neoforgeVersions = doNeoforge
    ? (async () => {
        /**
  ...
  for (const [list, broken, loader] of /** @type {[{ mcVersion: string }[], string[], string][]} */ ([
    [await fabricVersions, brokenLoaderVersions.fabric, 'fabric'],
    [await forgeVersions, brokenLoaderVersions.forge, 'forge'],
    [await neoforgeVersions, brokenLoaderVersions.neoforge, 'neoforge']
  ]))
    for (const brokenVersion of [...broken, ...brokenLoaderVersions.all]) {
      const index = list.findIndex(version => version.mcVersion === brokenVersion)
      if (index !== -1) {
